#!/usr/bin/env python3
"""
Load May 2025 Data with Enhanced Batting Statistics
"""
...
def load_may_data():
    ...
    print("🚀 LOADING May 2025 DATA WITH ENHANCED BATTING STATS")
    print("=" * 60)
    ...
    try:
        ...
        # 1. First, check if we need to clear existing May data
        print("1. Checking existing May 2025 data...")
        existing_may = db.fetch_results("""
        SELECT COUNT(*) as count
        FROM games 
        WHERE game_date >= '2025-05-01' AND game_date <= '2025-05-31'
        """)[0][0]
        
        if existing_may > 0:
            print(f"   Found {existing_may} existing May games")
            print("   Clearing existing May data to reload with enhanced stats...")
            ...
            delete_queries = [
                ...
                WHERE game_id IN (
                    ...
                    WHERE game_date >= '2025-05-01' AND game_date <= '2025-05-30'
                )
                """,
                """
                DELETE FROM games 
            ...
            print("   ✅ Existing May data cleared")
        else:
            print("   No existing May data found")
        
        # 2. Find all May combined data files
        may_data_path = Path("data/json/2025/05-May")
        combined_files = list(may_data_path.glob("combined_data_*.json"))
        combined_files.sort()
        
        print(f"\n2. Found {len(combined_files)} May combined data files")
        
        if not combined_files:
            print("❌ No May data files found! Run extraction first.")
            return
        ...
        print(f"\n🎉 May 2025 data loading completed successfully!")
        
    except Exception as e:
        print(f"❌ Error loading May data: {e}")
