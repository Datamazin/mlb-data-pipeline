#!/usr/bin/env python3
"""
Load April 2025 Data with Enhanced Batting Statistics
"""
...
def load_april_data():
    ...
    print("🚀 LOADING APRIL 2025 DATA WITH ENHANCED BATTING STATS")
    print("=" * 60)
    ...
    try:
        ...
        # 1. First, check if we need to clear existing April data
        print("1. Checking existing April 2025 data...")
        existing_april = db.fetch_results("""
        ...
        if existing_april > 0:
            print(f"   Found {existing_april} existing April games")
            print("   Clearing existing April data to reload with enhanced stats...")
            
            # Clear boxscore data for April games
            db.execute_query("""
            ...
            # Clear games data for April
            db.execute_query("""
            DELETE FROM games 
            WHERE game_date >= '2025-04-01' AND game_date <= '2025-04-30'
            """)
            
            print("   ✅ Existing April data cleared")
        else:
            print("   No existing April data found")
        
        # 2. Find all April combined data files
        april_data_path = Path("data/json/2025/04-April")
        combined_files = list(april_data_path.glob("combined_data_*.json"))
        combined_files.sort()
        
        print(f"\n2. Found {len(combined_files)} April combined data files")
        
        if not combined_files:
            print("❌ No April data files found! Run extraction first.")
            return
        ...
        if enhanced_stats[3] == boxscore_count:
        ...
        print(f"\n🎉 April 2025 data loading completed successfully!")
