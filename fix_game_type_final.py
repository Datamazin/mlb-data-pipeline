...
def fix_missing_game_types():
    ...
    try:
        # Get all games with missing game_type (separate transaction for reading)
        ...
        # Show final statistics using fresh connection
        final_db = DatabaseConnection()
        if final_db.connect():
            with final_db.connection.begin() as trans:
                result = final_db.connection.execute(text("""
