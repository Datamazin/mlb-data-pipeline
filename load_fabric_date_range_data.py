#!/usr/bin/env python3
"""
Load MLB Data to Microsoft Fabric with Enhanced Batting Statistics and Auto-Extraction

This script provides a complete end-to-end solution for loading MLB data to Microsoft Fabric:
...
4. Enhanced batting statistics validation with stolen bases and caught stealing

Features:
- Microsoft Fabric SQL Endpoint connectivity
...
- Enhanced batting statistics (doubles, triples, home runs, stolen bases, caught stealing)
...
def load_fabric_date_range_data(start_date, end_date, clear_existing=False, dry_run=False, create_folders=True, auto_extract=True):
    """Load MLB data for a dynamic date range to Microsoft Fabric with enhanced batting statistics.
    ...
    print(f"🚀 LOADING DATA TO MICROSOFT FABRIC FOR: {start_date} to {end_date}")
    print("=" * 70)
    ...
    try:
        if not dry_run:
            if not fabric_db.connect():
                print("❌ Failed to connect to Microsoft Fabric")
                print("💡 Make sure you have:")
                print("   - Valid Azure AD credentials")
                print("   - Access to the Fabric workspace")
                print("   - Correct server endpoint configured")
                return
        ...
        print(f"1. Analyzing existing Fabric data for {start_date} to {end_date}...")
        
        if not dry_run:
            existing_count = fabric_db.fetch_results("""
            SELECT COUNT(*) as count
            FROM games 
            WHERE game_date >= ? AND game_date <= ?
            """, (start_date, end_date))[0][0]
            
            print(f"   Found {existing_count} existing games in Fabric")
            
            if clear_existing and existing_count > 0:
                print("   🗑️  Clearing existing Fabric data to reload with enhanced stats...")
                ...
                fabric_db.execute_transaction(delete_queries)
                print("   ✅ Existing Fabric data cleared")
                existing_dates = set()  # No existing dates after clearing
            else:
                # Get list of dates that already exist in Fabric
                existing_dates_result = fabric_db.fetch_results("""
                ...
                if existing_dates:
                    print(f"   📅 Found data for {len(existing_dates)} dates already in Fabric")
                    if not clear_existing:
                        print("   🔄 Will only load missing dates (incremental loading)")
                        print("   💡 Use --clear to force reload all data")
                else:
                    print("   📂 No existing data found in Fabric - will load all files")
        else:
        ...
        print(f"     Files skipped: {len(skipped_files)} (already in Fabric)")
        ...
        if not date_folders:
            ...
            if len(skipped_files) > 0:
                print("✅ All files in date range already exist in Fabric!")
                print("💡 Use --clear to force reload existing data")
                if not dry_run:
                    # Still show verification of existing data
                    print(f"\n📊 Verifying existing Fabric data for {start_date} to {end_date}...")
                    
                    # Check games
                    games_count = fabric_db.fetch_results("""
                    SELECT COUNT(*) as count
                    FROM games 
                    WHERE game_date >= ? AND game_date <= ?
                    """, (start_date, end_date))[0][0]
                    print(f"   Games in Fabric: {games_count}")
                    
                    # Check boxscore records
                    boxscore_count = fabric_db.fetch_results("""
                    SELECT COUNT(*) as count
                    FROM boxscore b
                    INNER JOIN games g ON b.game_id = g.game_id
                    WHERE g.game_date >= ? AND g.game_date <= ?
                    """, (start_date, end_date))[0][0]
                    print(f"   Boxscore records in Fabric: {boxscore_count}")
                    
                    print(f"\n✅ No loading needed - data already exists in Fabric!")
                return
            else:
        ...
        if dry_run:
            print(f"\n🔍 DRY RUN SUMMARY:")
            print(f"   Would load {len(all_combined_files)} new files to Fabric")
        ...
        # 4. Initialize Fabric JSON loader with enhanced schema
        loader = FabricJSONLoader(fabric_db)
        
        # 5. Load each file
        print(f"\n3. Loading {len(all_combined_files)} files to Fabric...")
        ...
        for i, file_path in enumerate(all_combined_files, 1):
            try:
                print(f"   [{i}/{len(all_combined_files)}] Loading {file_path.name} to Fabric...")
                
                # Load the combined data file
                loader.load_json_to_database(str(file_path))
                success_count += 1
                
                if i % 25 == 0:  # Progress update every 25 files (Fabric may be slower)
        ...
        print(f"\n4. Loading to Fabric completed!")
        print(f"   ✅ Successfully loaded: {success_count} files")
        print(f"   ❌ Errors: {error_count} files")
        
        # 6. Verify the loaded data in Fabric
        print(f"\n5. Verifying loaded Fabric data for {start_date} to {end_date}...")
        ...
        WHERE game_date >= ? AND game_date <= ?
        """, (start_date, end_date))[0][0]
        print(f"   Games loaded to Fabric: {games_count}")
        ...
        # Check enhanced batting stats including stolen bases
        enhanced_stats = fabric_db.fetch_results("""
        SELECT 
        ...
        print(f"   Enhanced batting statistics in Fabric:")
        ...
        print(f"     Records with stolen bases: {enhanced_stats[3] or 0}")
        print(f"     Records with caught stealing: {enhanced_stats[4] or 0}")
        print(f"     Records with walks: {enhanced_stats[5] or 0}")
        print(f"     Total doubles: {enhanced_stats[6] or 0}")
        print(f"     Total triples: {enhanced_stats[7] or 0}")
        print(f"     Total home runs: {enhanced_stats[8] or 0}")
        print(f"     Total stolen bases: {enhanced_stats[9] or 0}")
        print(f"     Total caught stealing: {enhanced_stats[10] or 0}")
        print(f"     Total walks: {enhanced_stats[11] or 0}")
        
        if (enhanced_stats[6] or 0) > 0 or (enhanced_stats[7] or 0) > 0 or (enhanced_stats[8] or 0) > 0:
            print("   ✅ Enhanced batting stats loaded successfully to Fabric!")
        else:
            print("   ⚠️  Enhanced stats are all zero - this might be normal")
        
        # Date range summary
        actual_range = fabric_db.fetch_results("""
        ...
        if actual_range[0] and actual_range[1]:
            print(f"   📅 Actual date range in Fabric: {actual_range[0]} to {actual_range[1]}")
        
        print(f"\n🎉 Data loading to Microsoft Fabric completed successfully!")
        print(f"✅ Loaded {games_count} games and {boxscore_count} boxscore records")
        print(f"📊 Date range: {start_date} to {end_date}")
        print(f"🎯 Target: Microsoft Fabric SQL Endpoint")
        
    except Exception as e:
        print(f"❌ Error loading data to Fabric: {e}")
        import traceback
        traceback.print_exc()
...
def main():
    """Main function with command line argument parsing for Fabric loading."""
    parser = argparse.ArgumentParser(
        description="Load MLB data to Microsoft Fabric for a dynamic date range",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Load June 2025 data to Fabric (incremental - only new dates)
  python load_fabric_date_range_data.py --start 2025-06-01 --end 2025-06-30

  # Load entire 2025 season so far to Fabric (incremental, auto-create missing folders, auto-extract missing data)
  python load_fabric_date_range_data.py --start 2025-03-01 --end 2025-08-15

  # Add missing dates to existing Fabric range (auto-extracts if needed)
  python load_fabric_date_range_data.py --start 2025-04-01 --end 2025-05-31

  # Dry run to see what would be loaded and extracted
  python load_fabric_date_range_data.py --start 2025-06-01 --end 2025-06-30 --dry-run

  # Force clear existing Fabric data and reload everything  
  python load_fabric_date_range_data.py --start 2025-06-01 --end 2025-06-30 --clear
  
  # Load without automatically creating folders or extracting data
  python load_fabric_date_range_data.py --start 2025-08-01 --end 2025-08-15 --no-create-folders --no-auto-extract

Environment Variables Required:
  FABRIC_SERVER    - Your Fabric SQL Endpoint (e.g., xyz-workspace.datawarehouse.fabric.microsoft.com)
  FABRIC_DATABASE  - Database/Warehouse name (default: mlb_data)
  FABRIC_USERNAME  - Azure AD username (optional for integrated auth)
  FABRIC_PASSWORD  - Azure AD password (optional for integrated auth)
        """
    )
    ...
    parser.add_argument(
        '--clear', 
        action='store_true',
        help='Clear existing Fabric data in date range before loading (default: incremental loading)'
    )
    
    parser.add_argument(
        '--dry-run', 
        action='store_true',
        help='Show what would be loaded without actually loading data to Fabric'
    )
    ...
    try:
        ...
        if date_diff > 365:
            ...
            print("   This may take a long time to process and load to Fabric")
